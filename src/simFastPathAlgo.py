...
class SimFastPathAlgo(QObject):
    ...
    def __init__(self):
        super(SimFastPathAlgo, self).__init__()
        self.__stop = False
        self._fastestPath = []
            ...
            for i in range(-1, 2):
                for j in range(-1, 2):
                    ...
                    # to activate if using 8-directional movement
                        ...
                        continue
                    new_bearing = SimFastPathAlgo.get_bearing(cur, neighbor)
                    new_cost = cost_so_far[cur] + move_cost + SimFastPathAlgo.turn_cost(bearing[cur], new_bearing)
                    if neighbor not in cost_so_far or new_cost < cost_so_far[neighbor]:
                        cost_so_far[neighbor] = new_cost
                        # change heuristic if using 8-directional movement
                        priority = new_cost + SimFastPathAlgo.compute_manhattan_h(grid, neighbor)
    ...
    def gen_half_path(self, grid, start, goal):
        came_from, cost_so_far, bearing = SimFastPathAlgo.a_star(grid, start, goal)
        to = goal
        path = [to]
        cost = [cost_so_far[to]]
